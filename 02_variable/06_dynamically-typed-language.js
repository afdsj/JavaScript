/* 
정적 타입(static/strong type) 언어(c, c++, java, Kotlin)
...
이러한 문제는 개발의 유지보수성을 낮추는 경향이 있으며 디버깅 또한 쉽지 않다
이를 해결하고자 요즘 typeScript라는 자바스크립트에 타입을 추가해서 사용을 하고 있다
문법상의 큰 차이는 없으니 시간이 되면 공부를 하는게 좋다
*/
...
test = 1;
console.log(typeof test); // 타입 : number

test = 'javascript';
console.log(typeof test); // 타입 : string

test = true;
console.log(typeof test); // 타입 : boolean

test = null; // 자바스크립트 엔진 상의 오류여서 null 또한 object로 치환을 하고 있음
console.log(typeof test); // 타입 : object

test = Symbol(); // Symbole에 키를 안넣고 사용이 가능
console.log(typeof test); // 타입 : symbol
...
test = function(){};
console.log(typeof test); // 타입 : function
